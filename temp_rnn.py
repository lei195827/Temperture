...
if __name__ == '__main__':
    ...
    epoches = 30
    # 创建RNN模型
    model = RNN(input_size, hidden_size, num_layers, output_size).cuda()
    # criterion = nn.MSELoss()
    # 定义优化器
    criterion = nn.L1Loss()
    # optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, eps=1e-8, weight_decay=2e-5)
    optimizer = torch.optim.SGD(model.parameters(), lr=0.0002, momentum=0.9, weight_decay=2e-4)

    train_dataset = CustomDataset(all_feature, labels, sequence_length=sequence_length)
    test_dataset = CustomDataset(test_feature, test_labels, sequence_length=sequence_length)
